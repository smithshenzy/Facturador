        ...
        methods: {
           ...
           async initForm(){
            ...
            async  getRecords() {
                 ...
                 const response = await this.$http.get(`/reports/validate-documents/validate?${this.getQueryParameters()}`)
                 let responses=false;
                 if(response.data.success){
                    this.$message.success(response.data.message);
                        for (let index = 0; index < response.data.archivo_txt; index++) {
                             await  this.$http.get(`/reports/validate-documents/validateDocuments?numero=`+index).then((response) =>{
